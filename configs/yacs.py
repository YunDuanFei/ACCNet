...
def _load_cfg_from_file(file_obj):
    ...
    # file_obj <_io.TextIOWrapper name='configs/kins_snake.yaml' mode='r' encoding='UTF-8'>
    _, file_extension = os.path.splitext(file_obj.name)  # Separate file names from extensions
...
def _load_cfg_from_yaml_str(str_obj):
    ...
    cfg_as_dict = yaml.safe_load(str_obj)  # safe_load:Read the yaml file and return the dictionary
...
def _assert_with_logging(cond, msg):
    if not cond:
        logger.debug(msg)
    assert cond, msg  # The previous item is true, skip; The first one is false, execute assert statement two
